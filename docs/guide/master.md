...
符合atri规范的插件的详细操作请查阅服务：[插件商店](../service/plugin_store.md)。

::: warning
无法卸载 ATRI 自带的插件。如有卸载需求，你可以通过 `全局控制` 以全局禁用该插件或直接删除该插件。
:::

::: warning
ATRI 允许添加来自 [NoneBot2 Store](https://v2.nonebot.dev/store) 的插件，但不支持通过 nb-cli 控制，即意味着不支持通过 nb-cli 添加、删除来自 [NoneBot2 Store](https://v2.nonebot.dev/store) 的插件。

同时也不支持手动将插件文件移入 `plugins` 一类方式添加插件。所有未经过 ATRI Service 注册的插件将不被 ATRI 认可（代码层面禁用）。
:::
